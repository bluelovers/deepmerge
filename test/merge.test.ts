/**
 * Created by user on 2018/2/6/006.
 */

import { chai, relative, expect, path, assert, util } from './_local-dev';

// @ts-ignore
import { describe, before, beforeEach, it, ITest } from 'mocha';

import * as merge from '..';

// @ts-ignore
describe(relative(__filename), () =>
{
	let currentTest: ITest;

	beforeEach(function ()
	{
		currentTest = this.currentTest as ITest;

		//console.log('it:before', currentTest.title);
		//console.log('it:before', currentTest.fullTitle());
	});

	// @ts-ignore
	describe(`merge`, () =>
	{
		it('add keys in target that do not exist at the root', function(done) {
			var src = { key1: 'value1', key2: 'value2' }
			var target = {}

			var res = merge(target, src)

			assert.deepEqual(target, {}, 'merge should be immutable')
			assert.deepEqual(res, src)
			done()
		})

		it('merge existing simple keys in target at the roots', function(done) {
			var src = { key1: 'changed', key2: 'value2' }
			var target = { key1: 'value1', key3: 'value3' }

			var expected = {
				key1: 'changed',
				key2: 'value2',
				key3: 'value3',
			}

			assert.deepEqual(target, { key1: 'value1', key3: 'value3' })
			assert.deepEqual(merge(target, src), expected)
			done()
		})

		it('merge nested objects into target', function(done) {
			var src = {
				key1: {
					subkey1: 'changed',
					subkey3: 'added',
				},
			}
			var target = {
				key1: {
					subkey1: 'value1',
					subkey2: 'value2',
				},
			}

			var expected = {
				key1: {
					subkey1: 'changed',
					subkey2: 'value2',
					subkey3: 'added',
				},
			}

			assert.deepEqual(target, {
				key1: {
					subkey1: 'value1',
					subkey2: 'value2',
				},
			})
			assert.deepEqual(merge(target, src), expected)
			done()
		})

		it('replace simple key with nested object in target', function(done) {
			var src = {
				key1: {
					subkey1: 'subvalue1',
					subkey2: 'subvalue2',
				},
			}
			var target = {
				key1: 'value1',
				key2: 'value2',
			}

			var expected = {
				key1: {
					subkey1: 'subvalue1',
					subkey2: 'subvalue2',
				},
				key2: 'value2',
			}

			assert.deepEqual(target, { key1: 'value1', key2: 'value2' })
			assert.deepEqual(merge(target, src), expected)
			done()
		})

		it('should add nested object in target', function(done) {
			var src = {
				"b": {
					"c": {},
				},
			}

			var target = {
				"a": {},
			}

			var expected = {
				"a": {},
				"b": {
					"c": {},
				},
			}

			assert.deepEqual(merge(target, src), expected)
			done()
		})

		it('should clone source and target', function(done) {
			var src = {
				"b": {
					"c": "foo",
				},
			}

			var target = {
				"a": {
					"d": "bar",
				},
			}

			var expected = {
				"a": {
					"d": "bar",
				},
				"b": {
					"c": "foo",
				},
			}

			var merged = merge(target, src, { clone: true })

			assert.deepEqual(merged, expected)

			assert.notEqual(merged.a, target.a)
			assert.notEqual(merged.b, src.b)

			done()
		})

		it('should clone source and target', function(done) {
			var src = {
				"b": {
					"c": "foo",
				},
			}

			var target = {
				"a": {
					"d": "bar",
				},
			}

			var merged = merge(target, src)
			assert.notEqual(merged.a, target.a)
			assert.notEqual(merged.b, src.b)

			done()
		})

		it('should replace object with simple key in target', function(done) {
			var src = { key1: 'value1' }
			var target = {
				key1: {
					subkey1: 'subvalue1',
					subkey2: 'subvalue2',
				},
				key2: 'value2',
			}

			var expected = { key1: 'value1', key2: 'value2' }

			assert.deepEqual(target, {
				key1: {
					subkey1: 'subvalue1',
					subkey2: 'subvalue2',
				},
				key2: 'value2',
			})
			assert.deepEqual(merge(target, src), expected)
			done()
		})

		it('should replace objects with arrays', function(done) {
			var target = { key1: { subkey: 'one' } }

			var src = { key1: [ 'subkey' ] }

			var expected = { key1: [ 'subkey' ] }

			assert.deepEqual(merge(target, src), expected)
			done()
		})

		it('should replace arrays with objects', function(done) {
			var target = { key1: [ "subkey" ] }

			var src = { key1: { subkey: 'one' } }

			var expected = { key1: { subkey: 'one' } }

			assert.deepEqual(merge(target, src), expected)
			done()
		})

		it('should replace dates with arrays', function(done) {
			var target = { key1: new Date() }

			var src = { key1: [ "subkey" ] }

			var expected = { key1: [ "subkey" ] }

			assert.deepEqual(merge(target, src), expected)
			done()
		})

		it('should replace null with arrays', function(done) {
			var target = {
				key1: null,
			}

			var src = {
				key1: [ "subkey" ],
			}

			var expected = {
				key1: [ "subkey" ],
			}

			assert.deepEqual(merge(target, src), expected)
			done()
		})

		it('should work on simple array', function(done) {
			var src = [ 'one', 'three' ]
			var target = [ 'one', 'two' ]

			var expected = [ 'one', 'two', 'one', 'three' ]

			assert.deepEqual(merge(target, src), expected)
			assert.ok(Array.isArray(merge(target, src)))
			done()
		})

		it('should work on another simple array', function(done) {
			var target = [ "a1", "a2", "c1", "f1", "p1" ]
			var src = [ "t1", "s1", "c2", "r1", "p2", "p3" ]

			var expected = [ "a1", "a2", "c1", "f1", "p1", "t1", "s1", "c2", "r1", "p2", "p3" ]
			assert.deepEqual(target, [ "a1", "a2", "c1", "f1", "p1" ])
			assert.deepEqual(merge(target, src), expected)
			assert.ok(Array.isArray(merge(target, src)))
			done()
		})

		it('should work on array properties', function(done) {
			var src = {
				key1: [ 'one', 'three' ],
				key2: [ 'four' ],
			}
			var target = {
				key1: [ 'one', 'two' ],
			}

			var expected = {
				key1: [ 'one', 'two', 'one', 'three' ],
				key2: [ 'four' ],
			}

			assert.deepEqual(merge(target, src), expected)
			assert.ok(Array.isArray(merge(target, src).key1))
			assert.ok(Array.isArray(merge(target, src).key2))
			done()
		})

		it('should work on array properties with clone option', function(done) {
			var src = {
				key1: [ 'one', 'three' ],
				key2: [ 'four' ],
			}
			var target = {
				key1: [ 'one', 'two' ],
			}

			assert.deepEqual(target, {
				key1: [ 'one', 'two' ],
			})
			var merged = merge(target, src, { clone: true })

			assert.notEqual(merged.key1, src.key1)
			assert.notEqual(merged.key1, target.key1)
			assert.notEqual(merged.key2, src.key2)
			done()
		})

		it('should work on array of objects', function(done) {
			var src = [
				{ key1: [ 'one', 'three' ], key2: [ 'one' ] },
				{ key3: [ 'five' ] },
			]
			var target = [
				{ key1: [ 'one', 'two' ] },
				{ key3: [ 'four' ] },
			]

			var expected = [
				{ key1: [ 'one', 'two' ] },
				{ key3: [ 'four' ] },
				{ key1: [ 'one', 'three' ], key2: [ 'one' ] },
				{ key3: [ 'five' ] },
			]

			assert.deepEqual(merge(target, src), expected)
			assert.ok(Array.isArray(merge(target, src)), 'result should be an array')
			assert.ok(Array.isArray(merge(target, src)[0].key1), 'subkey should be an array too')

			done()
		})

		it('should work on array of objects with clone option', function(done) {
			var src = [
				{ key1: [ 'one', 'three' ], key2: [ 'one' ] },
				{ key3: [ 'five' ] },
			]
			var target = [
				{ key1: [ 'one', 'two' ] },
				{ key3: [ 'four' ] },
			]

			var expected = [
				{ key1: [ 'one', 'two' ] },
				{ key3: [ 'four' ] },
				{ key1: [ 'one', 'three' ], key2: [ 'one' ] },
				{ key3: [ 'five' ] },
			]

			var merged = merge(target, src, { clone: true })
			assert.deepEqual(merged, expected)
			assert.ok(Array.isArray(merge(target, src)), 'result should be an array')
			assert.ok(Array.isArray(merge(target, src)[0].key1), 'subkey should be an array too')
			assert.notEqual(merged[0].key1, src[0].key1)
			assert.notEqual(merged[0].key1, target[0].key1)
			assert.notEqual(merged[0].key2, src[0].key2)
			assert.notEqual(merged[1].key3, src[1].key3)
			assert.notEqual(merged[1].key3, target[1].key3)
			done()
		})

		it('should treat regular expressions like primitive values', function(done) {
			var target = { key1: /abc/ }
			var src = { key1: /efg/ }
			var expected = { key1: /efg/ }

			assert.deepEqual(merge(target, src), expected)

			expect(merge(target, src).key1.test('efg')).to.be.deep.equal(true);

			done()
		})

		it('should treat regular expressions like primitive values and should not'
			+ ' clone even with clone option', function(done) {
				var target = { key1: /abc/ }
				var src = { key1: /efg/ }

				var output = merge(target, src, { clone: true })

				assert.equal(output.key1, src.key1)
				done()
			}
		)

		it('should treat dates like primitives', function(done) {
			var monday = new Date('2016-09-27T01:08:12.761Z')
			var tuesday = new Date('2016-09-28T01:18:12.761Z')

			var target = {
				key: monday,
			}
			var source = {
				key: tuesday,
			}

			var expected = {
				key: tuesday,
			}
			var actual = merge(target, source)

			assert.deepEqual(actual, expected)
			assert.equal(actual.key.valueOf(), tuesday.valueOf())
			done()
		})

		it('should treat dates like primitives and should not clone even with clone'
			+  ' option', function(done) {
			var monday = new Date('2016-09-27T01:08:12.761Z')
			var tuesday = new Date('2016-09-28T01:18:12.761Z')

			var target = {
				key: monday,
			}
			var source = {
				key: tuesday,
			}

			var actual = merge(target, source, { clone: true })

			assert.equal(actual.key, tuesday)
			done()
		})

		it('should work on array with null in it', function(done) {
			var target = []

			var src = [ null ]

			var expected = [ null ]

			assert.deepEqual(merge(target, src), expected)
			done()
		})

		it('should clone array\'s element if it is object', function(done) {
			var a = { key: 'yup' }
			var target = []
			var source = [ a ]

			var output = merge(target, source, { clone: true })

			assert.notEqual(output[0], a)
			assert.equal(output[0].key, 'yup')
			done()
		})

		it('should clone an array property when there is no target array', function(done) {
			const someObject = {}
			var target = {}
			var source = { ary: [ someObject ] }
			var output = merge(target, source, { clone: true })

			assert.deepEqual(output, { ary: [{}] })
			assert.notEqual(output.ary[0], someObject)
			done()
		})

		it('should overwrite values when property is initialised but undefined', function(done) {
			var target1 = { value: [] }
			var target2 = { value: null }
			var target3 = { value: 2 }

			var src = { value: undefined }

			function hasUndefinedProperty(o) {
				assert.ok(o.hasOwnProperty('value'))
				expect(typeof o.value).to.be.deep.equal('undefined')
			}

			hasUndefinedProperty(merge(target1, src))
			hasUndefinedProperty(merge(target2, src))
			hasUndefinedProperty(merge(target3, src))

			done()
		})

		it('dates should copy correctly in an array', function(done) {
			var monday = new Date('2016-09-27T01:08:12.761Z')
			var tuesday = new Date('2016-09-28T01:18:12.761Z')

			var target = [ monday, 'dude' ]
			var source = [ tuesday, 'lol' ]

			var expected = [ monday, 'dude', tuesday, 'lol' ]
			var actual = merge(target, source)

			assert.deepEqual(actual, expected)
			done()
		})

	});
});
